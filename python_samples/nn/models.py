...
class TorchLinearFunction(torch.autograd.Function):
    ...
    def forward(ctx, x, wt, b):
        ...
        y = x @ wt + b # row-major = row-major x column-major
...
class ExtensionLinearFunction(torch.autograd.Function):
    ...
    def forward(ctx, x, wt, b):
        # Linear:
        # [B, out_features] = [B, in_features] x [in_features, out_features] + [out_features]
        y, _ = torch_extension.matmul_bias(x, wt, b) # row-major = row-major x column-major
        # Save for backward pass:
        ctx.save_for_backward(x, wt)
        return y
    ...
    def backward(ctx, do):
        ...
        x, wt = ctx.saved_tensors
        # Linear backprop:
        dx = torch_extension.matmul(do, wt.T)         # row-major = row-major x row-major (column-major transposed)
        dwt = torch_extension.matmul(do.T, x, True).T # column-major = column-major (row-major transposed) x row-major
        db = torch_extension.matrix_reduction_row(do) # sum reduction over rows
        return dx, dwt, db
...
class TorchLinearReLUFunction(torch.autograd.Function):
    ...
    def forward(ctx, x, wt, b):
        # Linear:
        # [B, out_features] = [B, in_features] x [in_features, out_features] + [out_features]    
        z = x @ wt + b # row-major = row-major x column-major
...
class ExtensionLinearReLUFunction(torch.autograd.Function):
    @staticmethod
    def forward(ctx, x, wt, b):
        # Linear with element wise ReLU activation::
        # [B, out_features] = [B, in_features] x [in_features, out_features] + [out_features]
        z, y = torch_extension.matmul_bias(x, wt, b, use_relu=True) # row-major, row-major = row-major x column-major
        # Save for backward pass:
        ctx.save_for_backward(x, wt, z)
        return y
    ...
    def backward(ctx, do):
        # Get saved tensors:
        x, wt, z = ctx.saved_tensors
        # Linear fused with ReLU backprop:
        # dz = do * (z > 0)  # do and dz are row-major
        dx = torch_extension.matmul_relu(do, z, wt.T) # row-major = row-major x row-major (column-major transposed)
        dwt = torch_extension.matmul_relu(do.T, z.T, x, True).T # column-major = column-major (row-major transposed) x row-major
        db = torch_extension.matrix_reduction_row_relu(do, z) # sum reduction over rows
        return dx, dwt, db
...
class CustomMLP(nn.Module):
    def __init__(self, in_features, out_features, use_extension, dtype = torch.float32):
        ...
        self.fc1 = CustomLinearReLU(in_features, 256, dtype=dtype, use_extension=use_extension, use_relu=True)
        self.fc2 = CustomLinearReLU(256, 128, dtype=dtype, use_extension=use_extension, use_relu=True)
