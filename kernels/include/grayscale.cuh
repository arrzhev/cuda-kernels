...
// Input data format for planar kernel - only kernel that can utilize vectorized memory access
template <typename T>
...
// Convert rgb to gray; Input matrix in interleaved format (Height x Width x Channel)
__global__ void rgb2grayInterleaved_kernel(const uchar3* src, unsigned char* dst, unsigned size);

// Convert rgb to gray using strides; Input matrix in any format (interleaved or planar)
...
// Convert rgb to gray; Input matrix in planar format (Channel x Height x Width)
// Can utilize vectorized memory access
template <ucharDim T>
