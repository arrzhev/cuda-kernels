...
__global__ void layerNorm_naive_kernel(const T *X, const T* W, const T *B, T *Y, float eps, unsigned size)
{
...
void launch_layerNorm_naive(const T *X, const T* W, const T *B, T *Y, float eps, unsigned rows, unsigned cols)
{
    ...
    if(cols % VEC_SIZE == 0U)
    {
        layerNorm_naive_kernel<blockDim, true><<<gridDim, blockDim>>>(X, W, B, Y, eps, cols);
    }
    else
    {
        layerNorm_naive_kernel<blockDim, false><<<gridDim, blockDim>>>(X, W, B, Y, eps, cols);
    }
}
