...
static void rgb2gray(const unsigned char* src_h, unsigned char* dst_h, unsigned size, bool isPlanar)
{
    unsigned char* src_d;
    unsigned char* dst_d;
    const unsigned dstByteSize = size * sizeof(unsigned char);
    const unsigned srcByteSize = 3 * dstByteSize; // src in rgb format, so 3 times more data
