...
torch::Tensor layerNorm(torch::Tensor X, torch::Tensor W, torch::Tensor B, float eps)
{
    ...
    launch_layerNorm<blockDim>(X.data_ptr<float>(), W.data_ptr<float>(), B.data_ptr<float>(), Y.data_ptr<float>(), eps, rows, cols);
    C10_CUDA_KERNEL_LAUNCH_CHECK();

    return Y;
}

torch::Tensor RMSNorm(torch::Tensor X, torch::Tensor W, float eps)
{
    CHECK_FP32(X);
    CHECK_FP32(W);
    CHECK_INPUT(X);
    CHECK_INPUT(W);
    TORCH_CHECK(X.dim() == 2 && W.dim() == 1, "tensor X must be 2 dim and W");
    TORCH_CHECK(X.size(1) == W.size(0), "Tensors must have same size");
    ...
    constexpr unsigned blockDim = 256U;
    launch_RMSNorm<blockDim>(X.data_ptr<float>(), W.data_ptr<float>(), Y.data_ptr<float>(), eps, rows, cols);
    C10_CUDA_KERNEL_LAUNCH_CHECK();
