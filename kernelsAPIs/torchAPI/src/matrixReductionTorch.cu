...
torch::Tensor matrixRowReduction(torch::Tensor A)
{
    ...
    if(useOpt)
        launch_matrixRowReduction<blockDim>(A.data_ptr<float>(), B.data_ptr<float>(), rows, cols);
    else
        launch_matrixRowReduction_naive<blockDim>(A.data_ptr<float>(), B.data_ptr<float>(), rows, cols);
